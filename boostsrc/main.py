...
ctypes.windll.kernel32.SetConsoleTitleW("Encrypted Services | .gg/encrypted")
...
@bot.event
async def on_ready():
    activity = discord.Game(name=".gg/encrypted", type=2)
    await bot.change_presence(activity=discord.Activity(type=discord.ActivityType.watching, name=" .gg/encrypted "))
    print(Fore.LIGHTBLACK_EX + "    ╔═╗┌┐┌┌─┐┬─┐┬ ┬┌─┐┌┬┐┌─┐┌┬┐  ╔═╗┌─┐┬─┐┬  ┬┬┌─┐┌─┐┌─┐")
    print(Fore.LIGHTRED_EX + "    ║╣ ││││  ├┬┘└┬┘├─┘ │ ├┤  ││  ╚═╗├┤ ├┬┘└┐┌┘││  ├┤ └─┐")
    print(Fore.RED + "    ╚═╝┘└┘└─┘┴└─ ┴ ┴   ┴ └─┘─┴┘  ╚═╝└─┘┴└─ └┘ ┴└─┘└─┘└─┘")
    print(Fore.YELLOW + "                     .gg/encrypted")
    print(f"\n           {Fore.RESET}[{Fore.BLUE} + {Fore.RESET}] {Fore.LIGHTBLACK_EX}Discord bot status | {Fore.RESET}[{Fore.GREEN}Online{Fore.RESET}]")
    print(f"           [{Fore.BLUE} + {Fore.RESET}] {Fore.LIGHTBLACK_EX}Big dick integration | {Fore.RESET}[{Fore.RED}Soon{Fore.RESET}]")
    print(f"               [{Fore.BLUE} + {Fore.RESET}] {Fore.LIGHTBLACK_EX}Proxy status | {Fore.RESET}[{Fore.GREEN}Online{Fore.RESET}]")
    print(f"\n          {Fore.LIGHTBLACK_EX} All services are running smoothly!")
...
async def on_message(message):
   embedVar = discord.Embed(title="CashApp", description="**CashApp Tag:** Encrypted Services *($Crutons)*")
   embedVar.set_thumbnail(url="https://cdn.discordapp.com/attachments/1009659952148774912/1014160903614234644/giphy.gif")
...
async def restock(ctx: discord.ApplicationContext, code: discord.Option(str, "pastebin code", required=True)):
    if not isAdmin(ctx):
        return await ctx.respond("*Only Bot Admins can use this command.*")

    await ctx.respond("Retrieving paste.")

    paste = requests.get(f"https://hastebin.com/raw/{code}", headers={
        ...
        "Referer": f"https://hastebin.com/raw/{code}",
...
if hwid in httpx.get("https://pastebin.com/GZqtfVnN").text:
